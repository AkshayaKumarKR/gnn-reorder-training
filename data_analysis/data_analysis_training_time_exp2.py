...
base_dir = '../experiments/Experiment 2 - Feature Size'

sets = {
    'Set 1 - Feature Size = 16': 'pyg',
    'Set 2 - Feature Size = 64': 'pyg',
    'Set 3 - Feature Size = 512': 'pyg'
}
...
feature_sizes = {'16': 'Set 1 - Feature Size = 16', '64': 'Set 2 - Feature Size = 64', '512': 'Set 3 - Feature Size = 512'}
strategies = sorted(list(combined_averaged_results[feature_sizes['16']].keys()))
...
# Plotting the bars for each feature size
rects1 = ax.bar(x - width, [combined_averaged_results[feature_sizes['16']][key] for key in strategies], width, label='Feature Size = 16')
rects2 = ax.bar(x, [combined_averaged_results[feature_sizes['64']][key] for key in strategies], width, label='Feature Size = 64')
rects3 = ax.bar(x + width, [combined_averaged_results[feature_sizes['512']][key] for key in strategies], width, label='Feature Size = 512')

# Add some text for labels, title, and custom x-axis tick labels, etc.
ax.set_xlabel('Graph Reordering Strategy')
ax.set_ylabel('Average Training Time (seconds)')
ax.set_title('Average Training Time by Graph Reordering Strategy and Feature Size')
ax.set_xticks(x)
...
# Save the figure
fig.savefig('training_time_comparison_exp2_feature_size_pyg.png', dpi=300, bbox_inches='tight')
