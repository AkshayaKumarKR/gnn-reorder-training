...
strategies = sorted(list(combined_averaged_results[dimensions['16']].keys()))
...
# Plotting the bars for each hidden dimension
rects1 = ax.bar(x - width, [combined_averaged_results[dimensions['16']][key] for key in strategies], width, label='Hidden Dimensions = 16')
rects2 = ax.bar(x, [combined_averaged_results[dimensions['64']][key] for key in strategies], width, label='Hidden Dimensions = 64')
rects3 = ax.bar(x + width, [combined_averaged_results[dimensions['512']][key] for key in strategies], width, label='Hidden Dimensions = 512')

# Add some text for labels, title, and custom x-axis tick labels, etc.
ax.set_xlabel('Graph Reordering Strategy')
ax.set_ylabel('Average Training Time (seconds)')
ax.set_title('Average Training Time by Graph Reordering Strategy and Hidden Dimension')
ax.set_xticks(x)
...
# Save the figure
fig.savefig('training_time_comparison_exp1_hidden_dimension_dgl.png', dpi=300, bbox_inches='tight')
