...
for graph in GRAPHS:
    for reordering in REORDERING_STRATAGIES:
        commands.append(f"python utils/compute_gap_metrics.py "
                        f"--graph_path input/{graph}/reordered-graphs/{graph}-{reordering}.dgl "
                        f"--reordering_strategy {reordering} --graph_name {graph}")
